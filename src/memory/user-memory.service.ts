...
export class UserMemoryService {
  ...
  async updateAfterRun(pubkey: string, ctx: RunContext): Promise<void> {
    try {
      ...
      const newAvg = ctx.amountSol
        ? (record.avgTradeSizeSol * record.runCount + ctx.amountSol) / totalRuns
      ...
      this.logger.warn(
        `Failed to update user memory for ${pubkey}: ${err?.message}`,
      );
    }
  }
  ...
  private buildSemanticChunkText(ctx: RunContext): string | null {
    ...
    if (
      typeof ctx.amountSol === 'number' &&
      Number.isFinite(ctx.amountSol) &&
      ctx.amountSol > 0
    ) {
