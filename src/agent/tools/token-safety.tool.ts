...
export class TokenSafetyTool implements KawulaTool {
    ...
    async execute(
        ...
        if (report.isHoneypot) warnings.push('honeypot detected — selling blocked by contract');
        if (!report.hasLiquidity) warnings.push('zero or locked liquidity');
        if (report.deployerRugged) warnings.push('deployer has rugged previous tokens');
    ...
    private async fetchRugCheck(mint: string): Promise<TokenSafetyReport> {
        try {
            ...
            if (!response.ok) {
                ...
                this.logger.warn(`RugCheck API returned ${response.status} for mint ${mint}`);
            ...
            const isHoneypot = risks.some((r) =>
                r.includes('honeypot') || r.includes('mint_authority'),
            );
            const hasLiquidity = (data?.markets?.length ?? 0) > 0;
            const deployerRugged = risks.some((r) =>
                r.includes('rugged') || r.includes('copycat'),
            );

            return { mint, riskScore, isHoneypot, hasLiquidity, deployerRugged, rugCheckRaw: data };
        } catch (err: any) {
