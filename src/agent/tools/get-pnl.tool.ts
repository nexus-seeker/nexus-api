...
export class GetPnlTool implements KawulaTool {
    ...
    readonly schema = {
        ...
        subject: '(Optional) Wallet address to analyse. Defaults to the user\'s own wallet.',
    };
    ...
    async execute(
        ...
        if (!ctx.heliusService) {
            return {
                success: false,
                rejectionReason: 'PnL summary requires HELIUS_API_KEY to be configured.',
    ...
    private classifyTx(tx: ParsedTransaction, walletPubkey: string): ClassifiedTx {
    ...
    private aggregate(txs: ClassifiedTx[], windowDays: number): PnlSummary {
        ...
        let largestOutflow: { amountSOL: number; description: string } | null = null;
        ...
        for (const tx of txs) {
            switch (tx.label) {
                ...
                case 'expense':
                    ...
                    if (!largestOutflow || Math.abs(tx.amountSol) > largestOutflow.amountSOL) {
                        largestOutflow = { amountSOL: Math.abs(tx.amountSol), description: tx.description };
                    }
                    break;
                case 'gas':
        ...
        const totalExpense = expenseSOL + gasFeeSOL + Math.abs(Math.min(swapNetSOL, 0));
    ...
    private formatMessage(s: PnlSummary, subject: string): string {
        const short = subject.length > 8 ? `${subject.slice(0, 4)}...${subject.slice(-4)}` : subject;
