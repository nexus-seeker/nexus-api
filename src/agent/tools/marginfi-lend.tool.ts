...
export class MarginfiLendTool implements KawulaTool {
  ...
  schema = {
    marginfi_lend:
      '{ "action": "marginfi_lend", "protocol": "marginfi", "amountSOL": 1.0, "token": "USDC" } // deposit tokens into Marginfi to earn lending yield',
  };
  ...
  async execute(
    ...
    this.logger.log(
      `Constructing Marginfi lend Tx for ${amountLamports} lamports of ${token}`,
    );
