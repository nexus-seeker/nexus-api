...
export class SwapTool implements KawulaTool {
  ...
  async execute(
    ...
    const tokenIn =
      (args.tokenIn as string | undefined)?.toUpperCase() ?? 'SOL';
    const tokenOut =
      (args.tokenOut as string | undefined)?.toUpperCase() ?? 'USDC';
    ...
    const sim = swapState.simulationResult ?? {
      fee: 5000,
      outAmount: 0,
      priceImpact: '0.00%',
    };
